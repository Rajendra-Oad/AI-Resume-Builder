...
@Repository class TemplateRepository {private final JdbcTemplate jdbc;TemplateRepository(JdbcTemplate j){jdbc=j;}
 ...
 void apply(String email,long templateId,long resumeId){get(templateId);int n=jdbc.update("UPDATE resumes r SET template_id=?,updated_at=CURRENT_TIMESTAMP FROM users u WHERE u.id=r.user_id AND r.id=? AND u.email=? AND r.deleted_at IS NULL",templateId,resumeId,email);if(n==0)throw new ResourceNotFoundException("Resume not found.");}
