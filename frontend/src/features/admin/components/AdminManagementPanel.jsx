...
export const AdminManagementPanel = () => {
  ...
  const update = useMutation({
    mutationFn: ({ kind, ...payload }) =>
      kind === "role" ? changeUserRole(payload) : changeUserStatus(payload),
  ...
  return (
    <ModulePage
      ...
      description="Manage user access, review activity, and maintain versioned AI prompts."
    >
                    ...
                    <tr key={user.id}>
                      ...
                      <td>
                        <Select
                          ...
                          onChange={(e) =>
                            update.mutate({ kind: "role", id: user.id, value: e.target.value })
                          }
                        >
                          <option>USER</option>
                      ...
                      <td>
                        <Select
                          ...
                          disabled={update.isPending}
                          onChange={(e) =>
                            update.mutate({ kind: "status", id: user.id, value: e.target.value })
                          }
                        >
        ...
        <div className="admin-activity-grid">
              ...
              <ul className="activity-list">
                {(actions.data?.items ?? []).map((item) => (
                  <li key={item.id}>
                    ...
                    <span>Target user {item.targetUserId ?? "—"}</span>
                    <time>{date(item.createdAt)}</time>
                  </li>
                ))}
              </ul>
              {!actions.data?.items?.length && (
                <p className="muted">No administrative actions recorded.</p>
              )}
            </Card>
          </AsyncState>
          <AsyncState
          ...
          >
            <Card>
              ...
              {!audit.data?.items?.length && <p className="muted">No audit entries recorded.</p>}
            </Card>
          </AsyncState>
        </div>
      )}
